...
pub trait RngSampler: RngCore {
    ...
    fn sample_index(&mut self, length: NonZeroUsize) -> usize {
        ...
        let index = floor(self.sample_uniform().get() * (length.get() as f64)) as usize;
    ...
    fn sample_index_u32(&mut self, length: NonZeroU32) -> u32 {
        ...
        let index = floor(self.sample_uniform().get() * f64::from(length.get())) as u32;
        index.min(length.get() - 1)
    }
    ...
    fn sample_index_u128(&mut self, length: NonZeroU128) -> u128 {
        ...
        let index = floor(self.sample_uniform().get() * (length.get() as f64)) as u128;
        index.min(length.get() - 1)
    }
